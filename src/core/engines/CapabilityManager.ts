  ...
  start(ctx: EngineContext): void {
    ...
    this.registry.set("show_dashboard", {
      ...
      handler: async (args) => {
        ...
        // cramped or cut off.
        const tooManyItems = finalItems.length > 4;
        const tooManyBars = Boolean(chart && chart.values.length > 5);
        const size = requestedSize === "quadrant" && (tooManyItems || tooManyBars) ? "full" : requestedSize;
